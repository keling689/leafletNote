...
L.MagnifyingGlass = L.Layer.extend({
  ...
  // 自己写的方法 放在第三个参数 切换放大镜位置用
  setGlassLatLng: function (latLng) {
    let layerPoint = this._mainMap.latLngToLayerPoint(latLng);
    this._wrapperElt.style.left = layerPoint.x - this.options.radius - 130 + 'px';
    this._wrapperElt.style.top = layerPoint.y - this.options.radius - 240 + 'px';
  },
  ...
  _update: function (latLng, layerPoint) {
    ...
    // this._wrapperElt.style.left = layerPoint.x - this.options.radius - 120 + 'px';
    // this._wrapperElt.style.top = layerPoint.y - this.options.radius - 230 + 'px';
    // console.log('this', this)
    // this._wrapperElt.style.left = 0 + 'px';
    // this._wrapperElt.style.top = 3 + 'px';
  },
